...
class UploadFarmEventHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if len(monitor_comparison) == 1:
            try:
                outcome = check_md5_and_process(
                    monitor_comparison, full_filename, self.backup_dir
                )
                if outcome == True:
...
def check_md5_and_process(monitor_comparison, full_filename, backup_dir, **kwargs):
    ...
    if current_md5 != info.get("md5", None) or (
        "force" in kwargs and kwargs.get("force") == True
    ):
