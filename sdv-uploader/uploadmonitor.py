...
class UploadMonitor:
    def __init__(self, **kwargs):
        self.time_between_checks = (
            5 if "time_between_checks" not in kwargs else kwargs["time_between_checks"]
        )
        self.error_signal = (
            None if "error_signal" not in kwargs else kwargs["error_signal"]
        )
        self.update_signal = (
            None if "update_signal" not in kwargs else kwargs["update_signal"]
        )
