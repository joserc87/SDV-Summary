...
from database import (
    check_settings,
    get_current_savegame_filenames,
    set_monitors,
    get_monitors,
    update_monitor,
    is_user_info_invalid,
    clear_user_info,
    get_latest_log_entry_for,
    get_user_info,
    set_user_info,
)
...
class GifferWindow(QMainWindow):
    ...
    def set_bg_image(self):
        self.bgimage = QtGui.QImage(resource_path("images/bg2.png")).scaled(
            self.size(), transformMode=QtCore.Qt.SmoothTransformation
        )
        palette = QtGui.QPalette()
        palette.setBrush(QtGui.QPalette.Window, QtGui.QBrush(self.bgimage))
    ...
    def gifpopup(self):
        QMessageBox.information(
            self,
            "GIF optimizing",
            "Your GIFs are not being optimized, which means they"
    ...
    def item_clicked_handler(self, item):
        ...
        if item.column() in column_fieldname:
            ...
            if checkstate != self._table_state[item.row()][item.column()]:
                if item.column() == 2:
                    update_monitor(
                        self._table_state[item.row()][0], monitoring=checkstate
                    )
                    if checkstate == True:
                        self._table.item(item.row(), 3).setFlags(
                        ...
                        self._table.item(item.row(), 3).setCheckState(
                            QtCore.Qt.Unchecked
                        )
                elif item.column() == 3:
                    update_monitor(
                        self._table_state[item.row()][0], uploading=checkstate
                    )
            ...
            if self._table_state[item.row()][item.column()] != None:
                QtGui.QDesktopServices.openUrl(
                    QUrl(
                        server_location
                        + "/"
                        + self._table_state[item.row()][item.column()]
                    )
                )
    ...
    def animate(self):
        ...
        if index.isValid():
            if self.user_uploads[row][4] <= 1:
                ...
                name = "{}, {} Farm, {}".format(
                    self._table_state[row][0],
                    self._table_state[row][1],
                    self._table_state[row][2],
                )
                # make_animation_in_process(name,self._table_state[row][4],annotated=True,type=anim_type,signal=self.renderComplete)
                at = AnimationThread(
                    name,
                    self._table_state[row][4],
                    annotated=annotated,
                    type=anim_type,
                    signal=self.renderComplete,
                    button=button,
                    duration=duration,
                )
...
class MainWindow(QMainWindow):
    ...
    def run_uploadmonitor(self):
        self._uploadmonitor = launch_uploadmonitor_as_thread(
            error_signal=self.authError, update_signal=self.updateGui
        )
    ...
    def init_tray(self):
        ...
        self.startupAction = QAction(
            "Start &Automatically", self, triggered=self.toggle_startup
        )
    ...
    def check_for_update(self):
        if not version_is_current():
            QMessageBox.information(
                self,
                "upload.farm uploader",
                "There is a new version of this tool! Please visit <a href='{}'>upload.farm</a> to download!".format(
                    server_location
                ),
            )
        else:
            QMessageBox.information(
                self,
                "upload.farm uploader",
                "Your uploader version appears up-to-date!",
            )
    ...
    def populate_table(self):
        ...
        for item in self._raw_db_output:
            try:
                ...
                info = []
                datestring = None
            j, uploadable, uploaded = get_latest_log_entry_for(
                item[0], successfully_uploaded=True
            )
            url = j.get("url", "..." if uploadable and not uploaded else None)
            row = [
                item[0],
                datestring,
                True if item[4] == 1 else False,
            ...
            if type(item) != bool:
                ...
                if i == 3 and items[2] == False:
                    ...
                    new_item.setCheckState(
                        QtCore.Qt.Unchecked if item == False else QtCore.Qt.Checked
                    )
            self._table.setItem(new_row - 1, i, new_item)
    ...
    def _logout(self):
        reply = QMessageBox.question(
            self,
            "Really logout?",
            "Are you sure you want to log out? Your API credentials will be deleted"
            " and you will have to re-authorise with upload.farm to continue using the uploader!",
