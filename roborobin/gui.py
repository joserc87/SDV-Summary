...
class MainWindow(QMainWindow):
	...
	def _create_layouts_and_widgets(self):
		...
		self._explanation = QLabel("This tool allows you to modify your v1.3+ Stardew Valley savegame to add extra cabins."
		...
		self.buttons = [self._add_log_cabin,self._add_plank_cabin,self._add_stone_cabin,self._remove_last_cabin,
						self._load_button,self._help_button,self._save_button,self._xposslider,self._yposslider]
	...
	def get_file(self):
		self.filename = QtWidgets.QFileDialog.getOpenFileName(self, "Open Savegame", os.path.join(os.getenv('APPDATA'),'StardewValley','Saves'), "All Files (*.*)")[0]
		if self.filename != '':
			try:
				self.savegame = SaveGame(self.filename)
				self.can_edit = self.savegame.v1_3()
				if self.can_edit != True:
					QMessageBox.information(self, "Not a v1.3 savegame!",
						"The file {} does not appear to be savegame from Stardew Valley version 1.3 or later. This program will probably crash if you try to modify it! For instructions on how to join the v1.3 beta, <a href='https://stardewvalley.net/stardew-valley-v1-3-beta/'>click here</a>".format(self.filename))
				try:
					del self.nc
				...
				QMessageBox.information(self, "Unrecognised file!",
					"The file {} does not appear to be savegame from Stardew Valley and cannot be loaded. Have you tried to open SaveGameInfo by accident?".format(self.filename))
	...
	def _add_cabin(self,cabinname = None):
		...
		try:
			...
			QMessageBox.information(self, "Error adding cabin!",
	...
	def save_file(self):
		if hasattr(self,'filename') and self.filename != None:
			...
			QMessageBox.information(self, "Saved!",
					"Your modified savegame has been written to {}! A backup of the original is stored in that folder in case you want to undo your changes.".format(self.filename))
		else:
			QMessageBox.information(self, "No file to save!",
