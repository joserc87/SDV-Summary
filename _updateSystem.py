...
def check_email_verification():
	db = connect_db()
	cur = db.cursor()
	cur.execute('SELECT users.id FROM users WHERE users.email_confirmed IS NULL AND NOT EXISTS (SELECT todo.id FROM todo WHERE todo.playerid=CAST(users.id AS text))')
	...
	cur.executemany('INSERT INTO todo (task, playerid) VALUES ('+app.sqlesc+','+app.sqlesc+')',[('email_confirmation',user_id[0]) for user_id in user_ids])
	db.commit()
	return len(user_ids)
...
def check_voting_base():
	db = connect_db()
	cur = db.cursor()
	cur.execute('SELECT id FROM playerinfo WHERE positive_votes IS NULL AND negative_votes IS NULL')
