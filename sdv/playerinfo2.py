...
class GameInfo:
    ...
    def get_info(self):
        if not hasattr(self, "info"):
            ...
            self.info["uniqueIDForThisGame"] = int(
                self.root.find("uniqueIDForThisGame").text
            )
            ...
            try:
                self.info["petName"] = (
                    self._get_npcs(petLocations, petTypes)[0].find("name").text
                )
            except IndexError:
                pass
            ...
            for player in players:
                ...
                if partner_id:
                    partner = next(
                        filter(
                            lambda x: x.get("UniqueMultiplayerID") == partner_id,
                            players,
                        )
                    )
    ...
    def _get_npcs(self, loc, types):
        ...
        for location in self.root.find("locations").iter("GameLocation"):
            if location.get(ns + "type") in loc:
                ...
                self._npcs += [
                    npc for npc in self._npc_temp if npc.get(ns + "type") in types
                ]
        return self._npcs
    ...
    def v1_3(self):
        if not hasattr(self, "_v1_3"):
            try:
                self._v1_3 = (
                    True
                    if str_to_bool(self.root.find("hasApplied1_3_UpdateChanges").text)
                    else False
                )
            except:
                self._v1_3 = False
        ...
        if not hasattr(self, "info"):
                ...
                try:
                    if self.player_node.find(tag).text != None:
                        ...
                        if tag == "professions":
                            ...
                            self.info["friendships"] = get_friendships(
                                self.player_node, self.v1_3
                            )
                        elif tag in ["hairstyleColor", "pantsColor", "newEyeColor"]:
                            self.info[tag] = list(
                                map(
                                    lambda c: int(
                                        self.player_node.find(tag).find(c).text
                                    ),
                                    "RGBA",
                                )
                            )
            ...
            if not self._portrait_info["partner"] and self.farmer_friendships:
                self._portrait_info["partner_id"] = get_multiplayer_partner(
                    self.info["UniqueMultiplayerID"], self.farmer_friendships
                )
            ...
            self._portrait_info["children"] = [
                (
                    int(child.find("gender").text),
