...
def checkApiAccess(userid):
    ...
    if len(r) > 0:
        ...
        return False
    client = ImgurClient(
        app.app.config["IMGUR_CLIENTID"], app.app.config["IMGUR_SECRET"]
    )
...
def getAuthUrl(userid, target=None):
    ...
    c.execute(
        "UPDATE users SET imgur_id=" + app.app.sqlesc + " WHERE id=" + app.app.sqlesc,
        (iuid, userid),
    )
    db.commit()
    db.close()
...
def swapCodeForTokens(response):
    ...
    if "error" in response:
        c.execute(
            "UPDATE users SET imgur_json=NULL WHERE imgur_id=" + app.app.sqlesc,
            (user_identifier,),
        )
        db.commit()
...
def uploadToImgur(userid, url):
    ...
    descriptionstring = (
        u"Stardew Valley game progress, full summary at http://upload.farm/{}".format(
            url
        )
    )
    # try:
    ...
    if app.app.config["IMGUR_DIRECT_UPLOAD"] == True:
        result = client.upload_from_path(
            map_url,
            config={"title": titlestring, "description": descriptionstring},
            anon=False,
        )
    else:
    ...
    imgur_json = json.dumps(
        {"imgur_url": "http://imgur.com/" + result["id"], "upload_time": time.time()}
    )
    c.execute(
        "UPDATE playerinfo SET imgur_json="
        + app.app.sqlesc
        + " WHERE url="
        + app.app.sqlesc,
        (imgur_json, url),
...
if __name__ == "__main__":
    ...
    response_looks_like = {
        u"datetime": 1460205330,
        u"bandwidth": 0,
        u"nsfw": None,
        u"vote": None,
        u"id": u"68htuuP",
        u"account_id": 33971158,
        u"in_gallery": False,
        u"title": u"The Title",
        u"section": None,
        u"width": 80,
        u"size": 1638,
        u"type": u"image/png",
        u"deletehash": u"yHKPz1regUue88y",
        u"description": u"The Description",
        u"views": 0,
        u"link": u"http://i.imgur.com/68htuuP.png",
        u"height": 152,
        u"name": u"",
        u"favorite": False,
        u"account_url": None,
        u"comment_preview": None,
        u"animated": False,
    }
