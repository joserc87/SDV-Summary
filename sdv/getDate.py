...
def preprocess_data(data):
    if (
        data.get("dayOfMonthForSaveGame") != None
        and int(data.get("dayOfMonthForSaveGame")) > 28
    ):
...
def get_date_data(statsDaysPlayed):
    ...
    yearForSaveGame = int(
        floor((statsDaysPlayed - dayOfMonthForSaveGame) / (28 * 4)) + 1
    )
    seasonForSaveGame = int(
        (floor((statsDaysPlayed - dayOfMonthForSaveGame) / 28))
        - ((yearForSaveGame - 1) * 4)
    )
...
def get_date(data):
    ...
    if (
        data.get("dayOfMonthForSaveGame") != None
        and data.get("seasonForSaveGame") != None
        and data.get("yearForSaveGame") != None
    ):
