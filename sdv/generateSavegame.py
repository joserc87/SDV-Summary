...
def genSaveGameInfo(savegame_file, read_data=False):
    ...
    savegameinfo = savegameinfo.replace(
        str_or_bytes(target_namespaces), str_or_bytes(required_namespaces), 1
    )
    return savegameinfo
...
def createZip(url, name, uniqueidforthisgame, static_folder, savegame_file):
    ...
    zf.writestr(
        os.path.join(folder, "SaveGameInfo"), genSaveGameInfo(savegamedata, True)
    )
    zf.writestr(
        "upload.farm_instructions.txt",
        "Downloaded from upload.farm/"
        + str(url)
