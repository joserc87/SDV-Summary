...
def generateFamilyPortrait(player_img, information, scale=4):
    ...
    if information['partner']:
        partner_img = Image.open(op.join(app.config['ASSET_FOLDER'], 'partners', '{0}.png'.format(information['partner'])))

    if information['cat']:
        pet_img = Image.open(op.join(app.config['ASSET_FOLDER'], 'pets', 'cat.png'))
    else:
        pet_img = Image.open(op.join(app.config['ASSET_FOLDER'], 'pets', 'dog.png'))
        ...
        child_imgs.append(
                    (Image.open(
                        op.join(
                            ...
                            'child', '{0}{1}{2}.png'.format(stage, gender, skin))), baby))
